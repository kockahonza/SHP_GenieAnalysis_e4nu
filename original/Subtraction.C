...
void Subtraction::prot3_rot_func(TVector3 V3prot[3], TVector3 V3prot_uncorr[3], TLorentzVector V4el,
                                 double Ecal_3pto2p[][2], double pmiss_perp_3pto2p[][2], double P3pto2p[][2],
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (prot_stat[0] && !prot_stat[1] && !prot_stat[2])
            N_p1[0] = N_p1[0] + 1;
        if (!prot_stat[0] && prot_stat[1] && !prot_stat[2])
            N_p1[1] = N_p1[1] + 1;
        if (!prot_stat[0] && !prot_stat[1] && prot_stat[2])
    ...
    for (int j = 0; j < N_3p; j++) { // looping through 1p combinations out of 3protons
        ...
        Ecal_3pto1p[j] =
            V4el.E() + TMath::Sqrt(m_prot * m_prot + V3prot[j].Mag() * V3prot[j].Mag()) - m_prot + bind_en[target_name];
        pmiss_perp_3pto1p[j] = TMath::Sqrt(V3_prot_el_3pto1p[j].Px() * V3_prot_el_3pto1p[j].Px() +
                                           V3_prot_el_3pto1p[j].Py() * V3_prot_el_3pto1p[j].Py());
    }
        ...
        for (int ind2 = 0; ind2 < N_3p; ind2++) {
            if (ind1 != ind2 && ind1 < ind2) {
                ...
                for (int g1 = 0; g1 < N_tot; g1++) {
                    ...
                    if (PFiducialCut(fbeam_en, V3_2p_rot[ind1]) && !PFiducialCut(fbeam_en, V3_2p_rot[ind2]))
                        N_p1det[count][0] = N_p1det[count][0] + 1;
                    if (!PFiducialCut(fbeam_en, V3_2p_rot[ind1]) && PFiducialCut(fbeam_en, V3_2p_rot[ind2]))
                        N_p1det[count][1] = N_p1det[count][1] + 1;
                    if (PFiducialCut(fbeam_en, V3_2p_rot[ind1]) && PFiducialCut(fbeam_en, V3_2p_rot[ind2]))
                        N_p1det[count][2] = N_p1det[count][2] + 1;
                }

                if (N_p1det[count][2] != 0 && N_pthree != 0) {

                    V3_prot_el_3pto2p[0] = V4el.Vect() + V3prot[ind1];
                    Ecal_3pto2p[count][0] = V4el.E() +
                                            TMath::Sqrt(m_prot * m_prot + V3prot[ind1].Mag() * V3prot[ind1].Mag()) -
                                            m_prot + bind_en[target_name];
                    pmiss_perp_3pto2p[count][0] = TMath::Sqrt(V3_prot_el_3pto2p[0].Px() * V3_prot_el_3pto2p[0].Px() +
                                                              V3_prot_el_3pto2p[0].Py() * V3_prot_el_3pto2p[0].Py());
                    P3pto2p[count][0] = N_p1det[count][0] / N_p1det[count][2] * (N_p2[count] / N_pthree);

                    V3_prot_el_3pto2p[1] = V4el.Vect() + V3prot[ind2];
                    Ecal_3pto2p[count][1] = V4el.E() +
                                            TMath::Sqrt(m_prot * m_prot + V3prot[ind2].Mag() * V3prot[ind2].Mag()) -
                                            m_prot + bind_en[target_name];
                    pmiss_perp_3pto2p[count][1] = TMath::Sqrt(V3_prot_el_3pto2p[1].Px() * V3_prot_el_3pto2p[1].Px() +
                                                              V3_prot_el_3pto2p[1].Py() * V3_prot_el_3pto2p[1].Py());
...
void Subtraction::prot2_rot_func(TVector3 V3prot[2], TVector3 V3prot_uncorr[2], TLorentzVector V4el,
    ...
    for (int g1 = 0; g1 < N_tot; g1++) {
        ...
        if (PFiducialCut(fbeam_en, V3_2prot[0]) && !PFiducialCut(fbeam_en, V3_2prot[1]))
            N_p2to1[0] = N_p2to1[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2prot[0]) && PFiducialCut(fbeam_en, V3_2prot[1]))
            N_p2to1[1] = N_p2to1[1] + 1;
        if (PFiducialCut(fbeam_en, V3_2prot[0]) && PFiducialCut(fbeam_en, V3_2prot[1]))
            N_2 = N_2 + 1;
    }
    ...
    Ecal_2pto1p[0] =
        V4el.E() + TMath::Sqrt(m_prot * m_prot + V3prot[0].Mag() * V3prot[0].Mag()) - m_prot + bind_en[target_name];
    pmiss_perp_2pto1p[0] = TMath::Sqrt(V3_prot_el_2pto1p[0].Px() * V3_prot_el_2pto1p[0].Px() +
                                       V3_prot_el_2pto1p[0].Py() * V3_prot_el_2pto1p[0].Py());

    V3_prot_el_2pto1p[1] = V4el.Vect() + V3prot[1];
    Ecal_2pto1p[1] =
        V4el.E() + TMath::Sqrt(m_prot * m_prot + V3prot[1].Mag() * V3prot[1].Mag()) - m_prot + bind_en[target_name];
    pmiss_perp_2pto1p[1] = TMath::Sqrt(V3_prot_el_2pto1p[1].Px() * V3_prot_el_2pto1p[1].Px() +
                                       V3_prot_el_2pto1p[1].Py() * V3_prot_el_2pto1p[1].Py());
...
void Subtraction::prot1_pi1_rot_func(TVector3 V3prot, TVector3 V3pi, int q_pi, double *N_pi_p, double *N_nopi_p) {
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_p_rot) && pi_stat)
            Npi_p = Npi_p + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !pi_stat)
            Nnopi_p = Nnopi_p + 1;
    }
...
void Subtraction::prot1_pi2_rot_func(TVector3 V3prot, TVector3 V3pi[2], int q_pi[2], double *P_1p0pi,
                                     double P_1p1pi[2]) {
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && !status_pi[1])
            N_1p1pi[0] = N_1p1pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && status_pi[1])
            N_1p1pi[1] = N_1p1pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && status_pi[1])
            N_all = N_all + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && !status_pi[1])
            Nnopi = Nnopi + 1;
    ...
    if (N_all != 0) {
        ...
        *P_1p0pi = Nnopi / N_all;
        ...
        for (int h = 0; h < N_pi; h++) {
            ...
            if (N_pi_p != 0)
                P_1p1pi[h] = (N_1p1pi[h] / N_all) * (N_nopi_p / N_pi_p);
            else
                P_1p1pi[h] = 0;
        }
    } // N_all!=0 statement
...
void Subtraction::prot1_pi3_rot_func(TVector3 V3prot, TVector3 V3pi[3], int q_pi[3], double *P_tot) {
    ...
    *P_tot = 0;
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && !status_pi[1] && !status_pi[2])
            N_1p1pi[0] = N_1p1pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && status_pi[1] && !status_pi[2])
            N_1p1pi[1] = N_1p1pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && !status_pi[1] && status_pi[2])
            N_1p1pi[2] = N_1p1pi[2] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && status_pi[1] && !status_pi[2])
            N_1p2pi[0] = N_1p2pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && !status_pi[1] && status_pi[2])
            N_1p2pi[1] = N_1p2pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && status_pi[1] && status_pi[2])
            N_1p2pi[2] = N_1p2pi[2] + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && status_pi[0] && status_pi[1] && status_pi[2])
            N_all = N_all + 1;
        if (PFiducialCut(fbeam_en, V3_p_rot) && !status_pi[0] && !status_pi[1] && !status_pi[2])
            Nnopi = Nnopi + 1;
    ...
    if (N_all != 0) {
        ...
        for (int h = 0; h < N_pi; h++) {
            ...
            prot1_pi1_rot_func(V3prot, V3pi[h], q_pi[h], &N_pi_p, &N_nopi_p);
            if (N_pi_p != 0)
                P_1p1pi = P_1p1pi + (N_1p1pi[h] / N_all) * (N_nopi_p / N_pi_p);
...
void Subtraction::prot2_pi1_rot_func(TVector3 V3_2prot_corr[2], TVector3 V3_2prot_uncorr[2], TVector3 V3_1pi, int q_pi,
                                     TLorentzVector V4_el, double Ecal_2p1pi_to2p0pi[2],
                                     double p_miss_perp_2p1pi_to2p0pi[2], double P_2p1pito2p0pi[2],
    ...
    *P_tot = 0;
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi1_stat)
            N_2p_0pi = N_2p_0pi + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi1_stat)
            N_1p_1pi[0] = N_1p_1pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi1_stat)
            N_1p_1pi[1] = N_1p_1pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi1_stat)
            N_1p_0pi[0] = N_1p_0pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi1_stat)
            N_1p_0pi[1] = N_1p_0pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi1_stat)
            N_all = N_all + 1;
    }
    ...
    if (N_all != 0) {
        ...
        prot2_rot_func(V3_2prot_corr, V3_2prot_uncorr, V4_el, Ecal_2p1pi_to2p0pi, p_miss_perp_2p1pi_to2p0pi, P_2pto1p,
                       &N_2p_det);
        ...
        for (int z = 0; z < N_2prot; z++) {
            ...
            if (N_pidet != 0)
                P_2p1pito1p1pi[z] = (N_1p_1pi[z] / N_all) * (N_piundet / N_pidet);
            else
                P_2p1pito1p1pi[z] = 0;
...
void Subtraction::prot2_pi2_rot_func(TVector3 V3_2prot_corr[2], TVector3 V3_2prot_uncorr[2], TVector3 V3_2pi[2],
                                     int q_pi[2], TLorentzVector V4_el, double Ecal_2p2pi[2],
    ...
    double rot_angle;
    double N_2p_0pi = 0, N_2p_1pi[N_2pi] = {0}, N_1p_2pi[N_2prot] = {0}, N_all = 0, N_1p_1pi[N_2prot][N_2pi] = {0},
           N_1p_0pi[N_2prot] = {0};
    ...
    double P_2p2pito1p0pi[N_2prot] = {0}, P_2p2pito1p1pi[N_2prot] = {0}, P_2p2pito1p2pi[N_2prot] = {0},
           P_2p2pito2p1pi[N_2prot] = {0};
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] &&
            !pi2_stat[1])
            N_2p_1pi[0] = N_2p_1pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
            pi2_stat[1])
            N_2p_1pi[1] = N_2p_1pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
            !pi2_stat[1])
            N_2p_0pi = N_2p_0pi + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] &&
            pi2_stat[1])
            N_1p_2pi[0] = N_1p_2pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] &&
            pi2_stat[1])
            N_1p_2pi[1] = N_1p_2pi[1] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] &&
            !pi2_stat[1])
            N_1p_1pi[0][0] = N_1p_1pi[0][0] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
            pi2_stat[1])
            N_1p_1pi[0][1] = N_1p_1pi[0][1] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] &&
            !pi2_stat[1])
            N_1p_1pi[1][0] = N_1p_1pi[1][0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
            pi2_stat[1])
            N_1p_1pi[1][1] = N_1p_1pi[1][1] + 1;
        if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && !PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
            !pi2_stat[1])
            N_1p_0pi[0] = N_1p_0pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && !pi2_stat[0] &&
    ...
    if (N_all != 0) {
        ...
        for (int z = 0; z < N_2prot; z++) {
            ...
            for (int k = 0; k < N_2pi; k++) {
                ...
                if (N_pidet != 0)
                    P_2p2pito1p1pi[z] = P_2p2pito1p1pi[z] + (N_1p_1pi[z][k] / N_all) * (N_piundet / N_pidet);
            }
            ...
            P_2p1pito1p0pi[0] = P_2p1pito1p0pi[1] = 0;
            Ptot = 0;
            prot2_pi1_rot_func(V3_2prot_corr, V3_2prot_uncorr, V3_2pi[z], q_pi[z], V4_el, Ecal_2p2pi, p_miss_perp_2p2pi,
                               P_2p1pito2p0pi, P_2p1pito1p1pi, P_2p1pito1p0pi, &Ptot);

            // P_2p2pito2p1pi[z]=(N_2p_1pi[0]/N_all)*(-P_2p1pito2p0pi[z]-
            // P_2p1pito1p1pi[z]+P_2p1pito1p0pi[z])+(N_2p_1pi[1]/N_all)*(-P_2p1pito2p0pi[z]-
            // P_2p1pito1p1pi[z]+P_2p1pito1p0pi[z]);
            // P_tot_2p[z]=-P_2p2pito1p0pi[z]+P_2p2pito1p1pi[z]+P_2p1pito2p0pi[z]+P_2p2pito1p2pi[z]+P_2p2pito2p1pi[z];

            // P_2p2pito2p1pi[z]=(N_2p_1pi[z]/N_all)*(-P_2p1pito2p0pi[0]-
            // P_2p1pito1p1pi[0]+P_2p1pito1p0pi[0])+(N_2p_1pi[z]/N_all)*(-P_2p1pito2p0pi[1]-
            // P_2p1pito1p1pi[1]+P_2p1pito1p0pi[1]);

            P_2p2pito2p1pi[0] = P_2p2pito2p1pi[0] +
                                (N_2p_1pi[z] / N_all) * (-P_2p1pito2p0pi[0] - P_2p1pito1p1pi[0] + P_2p1pito1p0pi[0]);
            P_2p2pito2p1pi[1] = P_2p2pito2p1pi[1] +
                                (N_2p_1pi[z] / N_all) * (-P_2p1pito2p0pi[1] - P_2p1pito1p1pi[1] + P_2p1pito1p0pi[1]);

        } // looping through 2p

        P_tot_2p[0] =
            -P_2p2pito1p0pi[0] + P_2p2pito1p1pi[0] + P_2p1pito2p0pi[0] + P_2p2pito1p2pi[0] + P_2p2pito2p1pi[0];
        P_tot_2p[1] =
            -P_2p2pito1p0pi[1] + P_2p2pito1p1pi[1] + P_2p1pito2p0pi[1] + P_2p2pito1p2pi[1] + P_2p2pito2p1pi[1];
...
void Subtraction::prot3_pi1_rot_func(TVector3 V3_3prot_corr[3], TVector3 V3_3prot_uncorr[3], TVector3 V3_pi, int q_pi,
                                     TLorentzVector V4_el, double Ecal_3p1pi[3], double p_miss_perp_3p1pi[3],
                                     double P_tot_3p[3]) {
    ...
    double N_1p0pi[N_3prot] = {0}, N_all = 0, N_1p1pi[N_3prot] = {0}, N_2p0pi[N_3prot] = {0}, N_2p1pi[N_3prot] = {0},
           N_3p0pi = 0;
    ...
    double E_cal_3pto2p[N_3prot][2] = {0}, p_miss_perp_3pto2p[N_3prot][2] = {0}, P_3pto2p[N_3prot][2] = {0},
           P_3p1pito3p0pi[N_3prot] = {0};
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            !PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_1p0pi[0] = N_1p0pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            !PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_1p0pi[1] = N_1p0pi[1] + 1;
        if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_1p0pi[2] = N_1p0pi[2] + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            !PFiducialCut(fbeam_en, V3_3p_rotated[2]) && pi_stat)
            N_1p1pi[0] = N_1p1pi[0] + 1;
        if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
        ...
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            !PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_2p0pi[0] = N_2p0pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_2p0pi[1] = N_2p0pi[1] + 1;
        if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_2p0pi[2] = N_2p0pi[2] + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            !PFiducialCut(fbeam_en, V3_3p_rotated[2]) && pi_stat)
            N_2p1pi[0] = N_2p1pi[0] + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && pi_stat)
            N_2p1pi[1] = N_2p1pi[1] + 1;
        if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && pi_stat)
            N_2p1pi[2] = N_2p1pi[2] + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
            PFiducialCut(fbeam_en, V3_3p_rotated[2]) && !pi_stat)
            N_3p0pi = N_3p0pi + 1;
        if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
    ...
    if (N_all != 0) {
        ...
        //----------------------------------3p 1pi ->3p 0pi->1p0pi   ----------------------------------------------
        prot3_rot_func(V3_3prot_uncorr, V3_3prot_corr, V4_el, E_cal_3pto2p, p_miss_perp_3pto2p, P_3pto2p, N_p1,
                       Ecal_3p1pi, p_miss_perp_3p1pi, &N_p_three);
        ...
        for (int z = 0; z < N_3prot; z++) {
            ...
            if (N_pidet != 0)
                P_3p1pito1p1pi[z] = (N_1p1pi[z] / N_all) * (N_piundet / N_pidet);
            ...
            for (int i = 0; i < N_3prot; i++) { // looping through 2p combinations  out of 3p
                if (z != i && z < i) {          // 3 pairs of 2proton combinations with z, i indexes(z<i)
                    ...
                    V3_2p_corr[0] = V3_3prot_corr[z];
                    V3_2p_corr[1] = V3_3prot_corr[i];
                    V3_2p_uncorr[0] = V3_3prot_uncorr[z];
                    V3_2p_uncorr[1] = V3_3prot_uncorr[i];
                    ...
                    prot2_pi1_rot_func(V3_2p_corr, V3_2p_uncorr, V3_pi, q_pi, V4_el, Ecal_2p1pi, p_miss_perp_2p1pi,
                                       P_2p1pito2p0pi, P_2p1pito1p1pi, P_2p1pito1p0pi, &Ptot);

                    // P_3p1pito2p1pi[z]=(N_2p1pi[count]/N_all)*(-P_2p1pito2p0pi[z]-
                    // P_2p1pito1p1pi[z]+P_2p1pito1p0pi[z]);
                    // P_3p1pito2p1pi[i]=(N_2p1pi[count]/N_all)*(-P_2p1pito2p0pi[i]-
                    // P_2p1pito1p1pi[i]+P_2p1pito1p0pi[i]);

                    P_3p1pito2p1pi[z] =
                        P_3p1pito2p1pi[z] +
                        (N_2p1pi[count] / N_all) * (-P_2p1pito2p0pi[0] - P_2p1pito1p1pi[0] + P_2p1pito1p0pi[0]);
                    P_3p1pito2p1pi[i] =
                        P_3p1pito2p1pi[i] +
                        (N_2p1pi[count] / N_all) * (-P_2p1pito2p0pi[1] - P_2p1pito1p1pi[1] + P_2p1pito1p0pi[1]);
...
void Subtraction::pi1_rot_func(TVector3 V3_pi, int q_pi, double *P_pi) {
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (Pi_phot_fid_united(fbeam_en, V3_rot_pi, q_pi))
            N_pion = N_pion + 1;
    }

    if (N_pion != 0)
        *P_pi = (N_tot - N_pion) / N_pion;
    else
        *P_pi = 0;
...
void Subtraction::pi3_rot_func(TVector3 V3_pi[3], int q_pi[3], double *P_0pi, double P_1pi[3], double P_320[3],
                               double P_3210[][2]) {
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (status_pi[0] && !status_pi[1] && !status_pi[2])
            N_1pi[0] = N_1pi[0] + 1;
        if (!status_pi[0] && status_pi[1] && !status_pi[2])
            N_1pi[1] = N_1pi[1] + 1;
        if (!status_pi[0] && !status_pi[1] && status_pi[2])
...
void Subtraction::pi4_rot_func(TVector3 V3_pi[4], int q_pi[4], double *P_0pi, double *P_410, double *P_420,
                               double *P_4210, double *P_430, double *P_4310, double *P_4320, double *P_43210) {
    ...
    for (int g = 0; g < N_tot; g++) {
        ...
        if (status_pi[0] && !status_pi[1] && !status_pi[2] && !status_pi[3])
            N_1pi[0] = N_1pi[0] + 1; // 1pi or phot
        if (!status_pi[0] && status_pi[1] && !status_pi[2] && !status_pi[3])
            N_1pi[1] = N_1pi[1] + 1;
        if (!status_pi[0] && !status_pi[1] && status_pi[2] && !status_pi[3])
            N_1pi[2] = N_1pi[2] + 1;
        ...
        if (!status_pi[0] && !status_pi[1] && !status_pi[2] && !status_pi[3])
            N_nopi = N_nopi + 1; // 0 pi or phot
        if (status_pi[0] && status_pi[1] && status_pi[2] && status_pi[3])
            N_allpi = N_allpi + 1; // 4pi or phot
    }
        ...
        *P_0pi = N_nopi / N_allpi;
        ...
        for (int h = 0; h < N_pi; h++) {
            ...
            *P_43210 = *P_43210 + ((P_3210_pion[0][0] + P_3210_pion[0][1]) + (P_3210_pion[1][0] + P_3210_pion[1][1]) +
                                   (P_3210_pion[2][0] + P_3210_pion[2][1])) *
                                      (N_3pi[h] / N_allpi);
