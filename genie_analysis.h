...
class genie_analysis {
  public:
    ...
    int fTorusCurrent;
    int fchoice;
    int NumOfProton; // Lataling: Define the variables here (explained below)
    int detector_acceptance;
    ...
    // respectively. 0 meaning no detector acceptance and 1 meaning or with detector acceptance (behaves likes a
    // boolean)
    genie_analysis(std::string, std::string, int, int, int, int, double, double, double, int, int, double, double,
                   double, double, int, int, TTree *tree = 0);
    ...
    void SetFiducialCutParameters(std::string beam_en) { fiducialcut->SetFiducialCutParameters(beam_en); }
    Bool_t EFiducialCut(std::string beam_en, TVector3 momentum) { return fiducialcut->EFiducialCut(beam_en, momentum); }
    Bool_t PFiducialCut(std::string beam_en, TVector3 momentum) { return fiducialcut->PFiducialCut(beam_en, momentum); }
    ...
    bool Phot_fid(TVector3 V3_phot) { return fiducialcut->Phot_fid(V3_phot); }
    ...
    void prot3_rot_func(std::string beam_en, TVector3 V3q, TVector3 V3prot[3], TVector3 V3prot_uncorr[3],
                        TLorentzVector V4el, double Ecal_3pto2p[][2], double pmiss_perp_3pto2p[][2],
                        double P3pto2p[][2], double N_p1[3], double Ecal_3pto1p[3], double pmiss_perp_3pto1p[3],
                        double *N_p3det);
    ...
    void prot2_pi2_rot_func(std::string beam_en, TVector3 V3_q, TVector3 V3_2prot_corr[2], TVector3 V3_2prot_uncorr[2],
                            TVector3 V3_2pi[2], int q_pi[2], bool radstat[2], TLorentzVector V4_el,
    ...
    void pi2_rot_func(std::string beam_en, TVector3 V3_pi[2], int q_pi[2], bool radstat[2], TVector3 V3_q,
                      double *P_0pi, double P_1pi[2]);
    void pi3_rot_func(std::string beam_en, TVector3 V3_pi[3], int q_pi[3], bool radstat[3], TVector3 V3_q,
                      double *P_0pi, double P_1pi[3], double P_320[3], double P_3210[][2]);
    void pi4_rot_func(std::string beam_en, TVector3 V3_pi[4], int q_pi[4], bool radstat[4], TVector3 V3_q,
                      double *P_0pi, double *P_410, double *P_420, double *P_4210, double *P_430, double *P_4310,
                      double *P_4320, double *P_43210);
...
genie_analysis::genie_analysis(std::string a_target, std::string a_beam_en, int number_rotations, int choice,
    ...
    std::cout << "Proton Momentum lb = " << a_protMom_lb << "\n" << std::endl;
    std::cout << "Proton Momentum ub = " << a_protMom_ub << "\n" << std::endl;
    std::cout << "Number of Protons = " << a_NumOfProton << "\n" << std::endl;
    std::cout << "Acceptance = " << a_detector_acceptance << "\n" << std::endl;
    ...
    if (tree == 0) {
        ...
        fTorusCurrent = 0;
        fchoice = choice;
        NumOfProton = a_NumOfProton; // Lataling: re-defined them here
        detector_acceptance = a_detector_acceptance;

        if (a_elSectors == 2) {
            t_elSectors->SetTitle("El_sect_16");
            fApplyPhiSliceEl_Sectors16 = true;
            t_AnyCuts->SetVal(true);
        } else if (a_elSectors == 1) {
            t_elSectors->SetTitle("El_sect_126");
            fApplyPhiSliceEl_Sectors126 = true;
        } else if (a_elSectors == 0) {
            t_elSectors->SetTitle("El_sect_Nocut");
        } else {
            std::cout << "You have entered an invalid value for elSectors (parameter 5). Valid options are 0, 1, or 2. "
                         "The given value is "
                      << a_elSectors << "." << std::endl;
            std::cout << "If you are adding an option for elSectors (parameter 5), be sure to make the necessary "
                         "changes in genie_analysis.C, "
        ...
        if (a_protSectors == 1) {
            ...
            t_AnyCuts->SetVal(true);
            if (a_elSectors == 2) {
                t_protSectors->SetTitle("Prot_sect_43");
            }
            if (a_elSectors == 1) {
                t_protSectors->SetTitle("Prot_sect_453");
            }
        } else if (a_protSectors == 0) {
            t_protSectors->SetTitle("Prot_Sect_Nocut");
        } else {
            std::cout << "You have entered an invalid value for protSectors (parameter 10). Valid options are 0 or 1. "
                         "The given value is "
                      << a_protSectors << "." << std::endl;
            std::cout << "If you are adding an option for protSectors (parameter 10), be sure to make the necessary "
                         "changes in genie_analysis.C, "
                         "genie_analysis.h, and run_genie_analysis.C."
                      << std::endl;
        }
        ...
        if (a_deltaPhiEl == 1) {
            ...
            std::cout << "You have entered an invalid value for deltaPhiEl (parameter 6). Valid options are 0 or 1. "
                         "The given value is "
        ...
        if (a_deltaPhiProt == 1) {
            ...
            std::cout << "You have entered an invalid value for deltaPhiProt (parameter 11). Valid options are 0 or 1. "
                         "The given value is "
        ...
        if ((a_thetaEl_lb + a_thetaEl_ub) < 180 && a_thetaEl_lb < a_thetaEl_ub) {
            t_thetaEl_lb->SetVal(a_thetaEl_lb);
            t_thetaEl_ub->SetVal(a_thetaEl_ub);
        ...
        if ((a_thetaProt_lb + a_thetaProt_ub) < 180 && a_thetaProt_lb < a_thetaProt_ub) {
            t_thetaProt_lb->SetVal(a_thetaProt_lb);
            t_thetaProt_ub->SetVal(a_thetaProt_ub);
            fApplyThetaSliceProt = true;
            t_AnyCuts->SetVal(true);
        }
        // Proton momentum lower and  upper bounds
        if ((a_protMom_lb + a_protMom_ub) > 0 && a_protMom_lb < a_protMom_ub) {
            t_ProtMom_lb->SetVal(a_protMom_lb);
            t_ProtMom_ub->SetVal(a_protMom_ub);
            fApplyProtMomCut = true;
            t_AnyCuts->SetVal(true);
        }
        ...
        if (fchoice == 1) {
            ...
            //		  chain->Add(Form("/pnfs/genie/persistent/users/apapadop/e4v_SuSav2/Exclusive/electrons/%s_%sGeV/apapadop_Rad_SuSav2_%s_%sGeV.root",
            // ftarget.c_str(),fbeam_en.c_str(),ftarget.c_str(),fbeam_en.c_str()));

            // Ext_Int Radiation SuSav2
            //		  chain->Add(Form("/pnfs/genie/persistent/users/apapadop/e4v_SuSav2/Exclusive/electrons/%s_%sGeV/apapadop_UpdatedSchwingerRad_SuSav2_%s_%sGeV.root",
            // ftarget.c_str(),fbeam_en.c_str(),ftarget.c_str(),fbeam_en.c_str()));
        ...
        if (fchoice == 2) {
            ...
            //		chain->Add(Form("/pnfs/genie/persistent/users/apapadop/e4v_G2018/Exclusive/electrons/%s_%sGeV/apapadop_Rad_G2018_%s_%sGeV.root",
            // ftarget.c_str(),fbeam_en.c_str(),ftarget.c_str(),fbeam_en.c_str()));

            // Ext+Int Radiation G2018

            // chain->Add(Form("/pnfs/genie/persistent/users/apapadop/e4v_G2018/Exclusive/electrons/%s_%sGeV/apapadop_UpdatedSchwingerRad_G2018_%s_%sGeV.root",
            // ftarget.c_str(),fbeam_en.c_str(),ftarget.c_str(),fbeam_en.c_str()));
        }
