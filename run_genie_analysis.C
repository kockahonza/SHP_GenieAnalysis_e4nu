...
int main(int argc, char **argv) {
    ...
    if (choice != 2 && choice != 1 && choice != 0) {
        std::cout
            << "Unknown option for data type (parameter 3). It should be either 0 or 1 (or 2). The given value is "
            << choice << std::endl;
        return 0;
    }

    if (rotations <= 0) {
        std::cout << "Not a valid number for the number of rotations (parameter 4). It should be greater than 0. The "
                     "given value is "
                  << rotations << std::endl;
        return 0;
    }
    ...
    if (elSectors_flag != 2 && elSectors_flag != 1 && elSectors_flag != 0) {
        std::cout
            << "Unknown flag for electron sectors (parameter 5). It should be either 0, 1, or 2. The given value is "
            << elSectors_flag << std::endl;
        std::cout << "If you are wanting to add a new option for this parameter, make sure you propogate all changes "
                     "in genie_analysis.h, "
    ...
    if (thetaEl_lb < 0 || thetaEl_lb > 180) {
        std::cout << "Not a valid number for the electron theta lower bound (parameter 7). It should be between 0 and "
                     "180 (inclusive). The "
                     "given value is "
                  << thetaEl_lb << std::endl;
        return 0;
    ...
    if (thetaEl_ub < 0 || thetaEl_ub > 180) {
        std::cout << "Not a valid number for the electron theta upper bound (parameter 8). It should be between 0 and "
                     "180 (inclusive). The "
                     "given value is "
                  << thetaEl_ub << std::endl;
        return 0;
    ...
    if (thetaEl_lb > thetaEl_ub) {
        std::cout << "The electron theta lower bound (parameter 7) must be less than or equal to the electron theta "
                     "upper bound (parameter 8)."
                  << std::endl;
        return 0;
    }

    if (elMom_lb < 0) {
        std::cout << "Not a valid number for the electron momentum lower bound (parameter 9). It should be greater "
    ...
    if (thetaProt_lb < 0 || thetaProt_lb > 180) {
        std::cout << "Not a valid number for the proton theta lower bound (parameter 12). It should be between 0 and "
                     "180 (inclusive). The "
                     "given value is "
    ...
    if (thetaProt_ub < 0 || thetaProt_ub > 180) {
        std::cout << "Not a valid number for the proton theta upper bound (parameter 13). It should be between 0 and "
                     "180 (inclusive). The "
                     "given value is "
    ...
    if (protMom_lb < 0) {
        std::cout << "Not a valid number for the proton momentum lower bound (parameter 14). It should be greater than "
                     "or equal to 0. The "
                     "given value is "
                  << protMom_lb << std::endl;
        return 0;
    ...
    if (protMom_ub < 0) {
        std::cout << "Not a valid number for the proton momentum upper bound (parameter 15). It should be greater than "
                     "or equal to 0. The "
                     "given value is "
                  << protMom_ub << std::endl;
        return 0;
