...
void Subtraction::prot3_rot_func(TVector3 V3prot[3], TVector3 V3prot_uncorr[3], TLorentzVector V4el, double Ecal_3pto2p[][2],
                                 double pmiss_perp_3pto2p[][2], double P3pto2p[][2], double N_p1[3], double Ecal_3pto1p[3],
  ...
  for (int j = 0; j < N_3p; j++) { // looping through 1p combinations out of 3protons
    ...
    pmiss_perp_3pto1p[j] =
        TMath::Sqrt(V3_prot_el_3pto1p[j].Px() * V3_prot_el_3pto1p[j].Px() + V3_prot_el_3pto1p[j].Py() * V3_prot_el_3pto1p[j].Py());
  }
    ...
    for (int ind2 = 0; ind2 < N_3p; ind2++) {
      if (ind1 != ind2 && ind1 < ind2) {
        ...
        if (N_p1det[count][2] != 0 && N_pthree != 0) {
          ...
          Ecal_3pto2p[count][0] =
              V4el.E() + TMath::Sqrt(m_prot * m_prot + V3prot[ind1].Mag() * V3prot[ind1].Mag()) - m_prot + bind_en[target_name];
          pmiss_perp_3pto2p[count][0] =
              TMath::Sqrt(V3_prot_el_3pto2p[0].Px() * V3_prot_el_3pto2p[0].Px() + V3_prot_el_3pto2p[0].Py() * V3_prot_el_3pto2p[0].Py());
          P3pto2p[count][0] = N_p1det[count][0] / N_p1det[count][2] * (N_p2[count] / N_pthree);

          V3_prot_el_3pto2p[1] = V4el.Vect() + V3prot[ind2];
          Ecal_3pto2p[count][1] =
              V4el.E() + TMath::Sqrt(m_prot * m_prot + V3prot[ind2].Mag() * V3prot[ind2].Mag()) - m_prot + bind_en[target_name];
          pmiss_perp_3pto2p[count][1] =
              TMath::Sqrt(V3_prot_el_3pto2p[1].Px() * V3_prot_el_3pto2p[1].Px() + V3_prot_el_3pto2p[1].Py() * V3_prot_el_3pto2p[1].Py());
...
void Subtraction::prot2_rot_func(TVector3 V3prot[2], TVector3 V3prot_uncorr[2], TLorentzVector V4el, double Ecal_2pto1p[2],
                                 double pmiss_perp_2pto1p[2], double P2pto1p[2], double *Nboth) {
  ...
  pmiss_perp_2pto1p[0] =
      TMath::Sqrt(V3_prot_el_2pto1p[0].Px() * V3_prot_el_2pto1p[0].Px() + V3_prot_el_2pto1p[0].Py() * V3_prot_el_2pto1p[0].Py());
  ...
  pmiss_perp_2pto1p[1] =
      TMath::Sqrt(V3_prot_el_2pto1p[1].Px() * V3_prot_el_2pto1p[1].Px() + V3_prot_el_2pto1p[1].Py() * V3_prot_el_2pto1p[1].Py());
...
void Subtraction::prot2_pi2_rot_func(TVector3 V3_2prot_corr[2], TVector3 V3_2prot_uncorr[2], TVector3 V3_2pi[2], int q_pi[2],
  ...
  for (int g = 0; g < N_tot; g++) {
    ...
    if (PFiducialCut(fbeam_en, V3_2p_rotated[0]) && PFiducialCut(fbeam_en, V3_2p_rotated[1]) && pi2_stat[0] && !pi2_stat[1])
  ...
  if (N_all != 0) {
    ...
    for (int z = 0; z < N_2prot; z++) {
      ...
      P_2p1pito1p0pi[0] = P_2p1pito1p0pi[1] = 0;
      Ptot = 0;
      prot2_pi1_rot_func(V3_2prot_corr, V3_2prot_uncorr, V3_2pi[z], q_pi[z], V4_el, Ecal_2p2pi, p_miss_perp_2p2pi, P_2p1pito2p0pi,
                         P_2p1pito1p1pi, P_2p1pito1p0pi, &Ptot);

      // P_2p2pito2p1pi[z]=(N_2p_1pi[0]/N_all)*(-P_2p1pito2p0pi[z]-
      // P_2p1pito1p1pi[z]+P_2p1pito1p0pi[z])+(N_2p_1pi[1]/N_all)*(-P_2p1pito2p0pi[z]- P_2p1pito1p1pi[z]+P_2p1pito1p0pi[z]);
      ...
      // P_2p1pito1p1pi[0]+P_2p1pito1p0pi[0])+(N_2p_1pi[z]/N_all)*(-P_2p1pito2p0pi[1]- P_2p1pito1p1pi[1]+P_2p1pito1p0pi[1]);
...
void Subtraction::prot3_pi1_rot_func(TVector3 V3_3prot_corr[3], TVector3 V3_3prot_uncorr[3], TVector3 V3_pi, int q_pi, TLorentzVector V4_el,
  ...
  for (int g = 0; g < N_tot; g++) {
    ...
    if (!PFiducialCut(fbeam_en, V3_3p_rotated[0]) && !PFiducialCut(fbeam_en, V3_3p_rotated[1]) &&
        PFiducialCut(fbeam_en, V3_3p_rotated[2]) && pi_stat)
      N_1p1pi[2] = N_1p1pi[2] + 1;
    if (PFiducialCut(fbeam_en, V3_3p_rotated[0]) && PFiducialCut(fbeam_en, V3_3p_rotated[1]) && !PFiducialCut(fbeam_en, V3_3p_rotated[2]) &&
        !pi_stat)
  ...
  if (N_all != 0) {
    ...
    //----------------------------------3p 1pi ->3p 0pi->1p0pi   ----------------------------------------------
    prot3_rot_func(V3_3prot_uncorr, V3_3prot_corr, V4_el, E_cal_3pto2p, p_miss_perp_3pto2p, P_3pto2p, N_p1, Ecal_3p1pi, p_miss_perp_3p1pi,
                   &N_p_three);
    ...
    for (int z = 0; z < N_3prot; z++) {
      ...
      for (int i = 0; i < N_3prot; i++) { // looping through 2p combinations  out of 3p
        if (z != i && z < i) {            // 3 pairs of 2proton combinations with z, i indexes(z<i)
          ...
          prot2_pi1_rot_func(V3_2p_corr, V3_2p_uncorr, V3_pi, q_pi, V4_el, Ecal_2p1pi, p_miss_perp_2p1pi, P_2p1pito2p0pi, P_2p1pito1p1pi,
                             P_2p1pito1p0pi, &Ptot);
...
void Subtraction::pi4_rot_func(TVector3 V3_pi[4], int q_pi[4], double *P_0pi, double *P_410, double *P_420, double *P_4210, double *P_430,
