...
void Fiducial::SetFiducialCutParameters(std::string beam_en) {
    ...
    // Please refer to <A HREF="http://einstein.unh.edu/protopop/FiducialCuts/fc4E2.html">Fiducial Cuts</A> --
    // D.Protopopescu(UNH)
    std::string fbeam_en = beam_en;

    if (en_beam[fbeam_en] > 4. &&
        en_beam[fbeam_en] < 5.) { //
                                  // reads FC parameters for 4.4GeV , e- and p fiducial cut parameters at 4GeV
                                  //
        std::ifstream param_file2(Form("./FiducialsCorrections/PFID_%s_%d.dat", fbeam_en.c_str(),
                                       fTorusCurrent)); // reading the proton fiducial cut parameters at 4GeV
        ...
        while ((sector != 6 || param_type != 21)) {
            ...
            case 0:
                for (int k = 0; k < 2; k++)
                    fgPar_4Gev_2250_Efid_t0_p[sector - 1][k] = data[k];
                break;
            case 1:
                for (int k = 0; k < 6; k++)
                    fgPar_4Gev_2250_Efid_t1_p[sector - 1][k] = data[k];
                break;
            case 10:
                for (int k = 0; k < 6; k++)
                    fgPar_4Gev_2250_Efid_b_p[sector - 1][0][k] = data[k];
                break;
            case 11:
                for (int k = 0; k < 6; k++)
                    fgPar_4Gev_2250_Efid_b_p[sector - 1][1][k] = data[k];
                break;
            case 20:
                for (int k = 0; k < 6; k++)
                    fgPar_4Gev_2250_Efid_a_p[sector - 1][0][k] = data[k];
                break;
            case 21:
                for (int k = 0; k < 6; k++)
...
Bool_t Fiducial::GetEPhiLimits(std::string beam_en, Float_t momentum, Float_t theta, Int_t sector, Float_t *EPhiMin,
                               Float_t *EPhiMax) {
    // Begin_Html
    /*</pre>
     ...
     Please refer to <A HREF="http://www.jlab.org/Hall-B/secure/e2/bzh/efiducialcut.html">Electron Fiducial Cuts</A> --
    Bin Zhang (MIT). For 4.4GeV please refer to <A
    HREF="http://einstein.unh.edu/protopop/FiducialCuts/fc4E2.html">Fiducial Cuts</A> -- D.Protopopescu (UNH) <pre>
    */
    // End_Html
    std::string fbeam_en = beam_en;
    if (sector < 0 || sector > 5)
        return kFALSE; // bad input

    if (en_beam[fbeam_en] > 4. && en_beam[fbeam_en] < 5. && fTorusCurrent > 2240 &&
        fTorusCurrent < 2260) { // 4.4GeV fiducial cuts by protopop@jlab.org
        if ((theta < 15.) || (momentum < 0.9))
            return kFALSE; // out of range
        Float_t t0, t1, b[2], a[2];

        if (momentum > 3.7)
            momentum = 3.7; // don't extrapolate past the data
        ...
        t1 = 0.;
        for (int k = 0; k < 6; k++)
            t1 += (fgPar_4Gev_2250_Efid_t1_p[sector][k] * pow(momentum, k));
        for (int l = 0; l < 2; l++) {
            b[l] = 0.;
            for (int k = 0; k < 6; k++)
                b[l] += (fgPar_4Gev_2250_Efid_b_p[sector][l][k] * pow(momentum, k));
            a[l] = 0.;
            for (int k = 0; k < 6; k++)
                a[l] += (fgPar_4Gev_2250_Efid_a_p[sector][l][k] * pow(momentum, k));
        }
...
Bool_t Fiducial::EFiducialCut(std::string beam_en, TVector3 momentum) {
    ...
    if (en_beam[fbeam_en] > 1. && en_beam[fbeam_en] < 2. && fTorusCurrent > 740 && fTorusCurrent < 1510) {
        ...
        if (p_thetae > 1.05) {
            p_thetae = 1.05;
        } else if (p_thetae < 0.4) {
            p_thetae = 0.4;
        }

        for (int i = 4; i >= 0; i--) {
            thetamax_e = thetamax_e * p_thetae + el_thetamax1[i];
        }
        ...
        if (SCpdcut && (fTorusCurrent > 1490) &&
            (fTorusCurrent <
             1510)) { // if the SCpdCut bit is set, take off the bad SC paddle by strictly cutting off a theta gap.
            ...
            if (status) {
                ...
                if (tsector == 3) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        for (int d = 7; d >= 0; d--) {
                            badpar3[i] = badpar3[i] * mom + fgPar_1gev_1500_Efid_Theta_S3[i][d];
                        }
                    }
                ...
                // sector 4 has one bad paddle
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        for (int d = 7; d >= 0; d--) {
                            badpar4[i] = badpar4[i] * mom + fgPar_1gev_1500_Efid_Theta_S4[i][d];
                        }
                    }
                ...
                // sector 5 has four bad paddles
                    ...
                    for (Int_t i = 0; i < 8; i++) {
                        ...
                        for (Int_t d = 7; d >= 0; d--) {
                            badpar5[i] = badpar5[i] * mom + fgPar_1gev_1500_Efid_Theta_S5[i][d];
                        }
                    }
                    ...
                    if (mom < 1.27) {
                        badpar5[1] = 24.0 * 1500 / 2250;
                    } // some dummy constants. see fiducial cuts webpage.

                    for (Int_t ip = 0; ip < 4; ip++) {
                        status = status && !(theta > badpar5[2 * ip] && theta < badpar5[2 * ip + 1]);
                    }
                }
            }
        }
            ...
            if (status) {
                ...
                if (tsector == 3) {
                    ...
                    for (int d = 0; d < 4; d++) {
                        ...
                        parsec3_l[d] = fid_1gev_750_efid_S3[d][0][0] + fid_1gev_750_efid_S3[d][0][1] / mom +
                                       fid_1gev_750_efid_S3[d][0][2] / (mom * mom) +
                                       fid_1gev_750_efid_S3[d][0][3] / (mom * mom * mom);
                        parsec3_h[d] = fid_1gev_750_efid_S3[d][1][0] + fid_1gev_750_efid_S3[d][1][1] / mom +
                                       fid_1gev_750_efid_S3[d][1][2] / (mom * mom) +
                                       fid_1gev_750_efid_S3[d][1][3] / (mom * mom * mom);
                ...
                // sector 4 has two gaps , second gap appears only at p<0.3 and theta>105 and affects only pimi
                    ...
                    for (int d = 0; d < 2; d++) {
                        ...
                        parsec4_l[d] = fid_1gev_750_efid_S4[d][0][0] + fid_1gev_750_efid_S4[d][0][1] / mom +
                                       fid_1gev_750_efid_S4[d][0][2] / (mom * mom) +
                                       fid_1gev_750_efid_S4[d][0][3] / (mom * mom * mom);
                        parsec4_h[d] = fid_1gev_750_efid_S4[d][1][0] + fid_1gev_750_efid_S4[d][1][1] / mom +
                                       fid_1gev_750_efid_S4[d][1][2] / (mom * mom) +
                                       fid_1gev_750_efid_S4[d][1][3] / (mom * mom * mom);
                ...
                // sector 5 has three gaps
                    ...
                    for (int d = 0; d < 3; d++) {
                        ...
                        if (d == 0 && mom > 0.3) {
                            mom = 0.3;
                        } // first one shows up only for pimi at p<0.3 and theta~128
                        else if (d > 0 && mom < 0.5) {
                            mom = 0.5;
                        }
                        parsec5_l[d] = fid_1gev_750_efid_S5[d][0][0] + fid_1gev_750_efid_S5[d][0][1] / mom +
                                       fid_1gev_750_efid_S5[d][0][2] / (mom * mom) +
                                       fid_1gev_750_efid_S5[d][0][3] / (mom * mom * mom);
                        parsec5_h[d] = fid_1gev_750_efid_S5[d][1][0] + fid_1gev_750_efid_S5[d][1][1] / mom +
                                       fid_1gev_750_efid_S5[d][1][2] / (mom * mom) +
                                       fid_1gev_750_efid_S5[d][1][3] / (mom * mom * mom);
            ...
            if (status) {
                ...
                if (tsector == 3) { // sector 3 has two bad paddles
                    ...
                    if (mom < 1.25)
                        badpar5[0] = 23.4;
                    if (mom < 1.27)
                        badpar5[1] = 24.0; // some dummy constants. see fiducial cuts webpage.
                ...
                if (status) {
                    ...
                    if (tsector == 3) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            badpar3[i] = fgPar_4Gev_2250_Efid_Theta_S3_extra[i][0] +
                                         fgPar_4Gev_2250_Efid_Theta_S3_extra[i][1] / mom +
                                         fgPar_4Gev_2250_Efid_Theta_S3_extra[i][2] / (mom * mom) +
                                         fgPar_4Gev_2250_Efid_Theta_S3_extra[i][3] / (mom * mom * mom);
                        }
                    ...
                    // sector 4 has one bad paddle
                        ...
                        for (int i = 0; i < 2; i++) {
                            ...
                            badpar4[i] = fgPar_4Gev_2250_Efid_Theta_S4_extra[i][0] +
                                         fgPar_4Gev_2250_Efid_Theta_S4_extra[i][1] / mom +
                                         fgPar_4Gev_2250_Efid_Theta_S4_extra[i][2] / (mom * mom) +
                                         fgPar_4Gev_2250_Efid_Theta_S4_extra[i][3] / (mom * mom * mom);
                        }
                    ...
                    // sector 5 has four bad paddles
                        ...
                        for (Int_t i = 0; i < 8; i++) {
                            ...
                            badpar5[i] = fgPar_4Gev_2250_Efid_Theta_S5_extra[i][0] +
                                         fgPar_4Gev_2250_Efid_Theta_S5_extra[i][1] / mom +
                                         fgPar_4Gev_2250_Efid_Theta_S5_extra[i][2] / (mom * mom) +
                                         fgPar_4Gev_2250_Efid_Theta_S5_extra[i][3] / (mom * mom * mom);
                        }
...
Bool_t Fiducial::PFiducialCut(std::string beam_en, TVector3 momentum) {
    ...
    // Please refer to <A HREF="http://www.jlab.org/Hall-B/secure/e2/bzh/pfiducialcut.html">Electron Fiducial Cuts</A>
    // -- Bin Zhang (MIT).
    ...
    if (beam_en == "") {
            ...
            if (fTorusCurrent < 1510 && fTorusCurrent > 1490) {
                ...
                for (Int_t mompar = 0; mompar < 6; mompar++) {
                    ...
                    for (Int_t phipar = 0; phipar < 5; phipar++) {
                        ...
                        // std::cout << p << " " << mompar << " " << phipar << " " << phipars[1] << " " << phipars[2] <<
                        // std::endl; std::cout << " " << phipars[3] << " " << phipars[4] << " " << phipars[5] <<
                        // std::endl;
                    }
                }
                ...
                if (status && SCpdcut) { // cut bad scintillator paddles
                    ...
                    if (mom_scpd < 0.2)
                        mom_scpd = 0.2; // momentum smaller than 200 MeV/c, use 200 MeV/c
                    if (mom_scpd > 1.0)
                        mom_scpd = 1.0; // momentum greater than 1000 MeV/c, use 1000 MeV/c
                ...
                if (!PFiducialCutExtra(fbeam_en, momentum)) {
                    status = false;
                }
            ...
            if (fTorusCurrent < 760 && fTorusCurrent > 740) {
                ...
                for (Int_t mompar = 0; mompar < 6; mompar++) {
                    for (Int_t phipar = 0; phipar < 5; phipar++) {
                        ...
                        // std::cout << p << " " << mompar << " " << phipar << " " << phipars[1] << " " << phipars[2] <<
                        // " " << phipars[3] << " " << phipars[4] << " " << phipars[5] << std::endl;
                    }
                }
                ...
                if (p_theta > 0.95)
                    ...
                    p_theta = 0.1;
                for (int i = 4; i >= 0; i--)
                    thetamax_p = thetamax_p * p_theta + pipl_thetamax1[i];
                ...
                if (status && SCpdcut) { // cut bad scintillator paddles
                    ...
                    if (mom_scpd < 0.15)
                        mom_scpd = 0.15; // momentum smaller than 150 MeV/c, use 150 MeV/c

                    // sector 1 has two gaps , 1st gap appears only at smaller momenta and theta>110 and affects only
                    // pipl
                    if (tsector == 1) {
                        ...
                        for (int d = 0; d < 2; d++) {
                            ...
                            if (d == 0 && mom_scpd > 0.45)
                                mom_scpd = 0.45;
                            else if (d == 0 && mom_scpd < 0.15)
                                mom_scpd = 0.15;
                            else if (d == 1 && mom_scpd > 0.55)
                                mom_scpd = 0.55;
                            else if (d == 1 && mom_scpd < 0.3)
                                mom_scpd = 0.3;
                            parsec1_l[d] = fid_1gev_750_pfid_S1[d][0][0] + fid_1gev_750_pfid_S1[d][0][1] / mom_scpd +
                                           fid_1gev_750_pfid_S1[d][0][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S1[d][0][3] / (mom_scpd * mom_scpd * mom_scpd);
                            parsec1_h[d] = fid_1gev_750_pfid_S1[d][1][0] + fid_1gev_750_pfid_S1[d][1][1] / mom_scpd +
                                           fid_1gev_750_pfid_S1[d][1][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S1[d][1][3] / (mom_scpd * mom_scpd * mom_scpd);
                    ...
                    // sector 2 has 1 gap1
                        ...
                        double parsec2_l, parsec2_h;
                        parsec2_l = fid_1gev_750_pfid_S2[0][0] + fid_1gev_750_pfid_S2[0][1] / mom_scpd +
                                    fid_1gev_750_pfid_S2[0][2] / (mom_scpd * mom_scpd) +
                                    fid_1gev_750_pfid_S2[0][3] / (mom_scpd * mom_scpd * mom_scpd);
                        parsec2_h = fid_1gev_750_pfid_S2[1][0] + fid_1gev_750_pfid_S2[1][1] / mom_scpd +
                                    fid_1gev_750_pfid_S2[1][2] / (mom_scpd * mom_scpd) +
                                    fid_1gev_750_pfid_S2[1][3] / (mom_scpd * mom_scpd * mom_scpd);
                        status = status && !(theta > parsec2_l && theta < parsec2_h);
                    }
                    ...
                    // sector 3 has four gaps
                        ...
                        double parsec3_l[4], parsec3_h[4];
                        for (int d = 0; d < 4; d++) {
                            parsec3_l[d] = fid_1gev_750_pfid_S3[d][0][0] + fid_1gev_750_pfid_S3[d][0][1] / mom_scpd +
                                           fid_1gev_750_pfid_S3[d][0][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S3[d][0][3] / (mom_scpd * mom_scpd * mom_scpd);
                            parsec3_h[d] = fid_1gev_750_pfid_S3[d][1][0] + fid_1gev_750_pfid_S3[d][1][1] / mom_scpd +
                                           fid_1gev_750_pfid_S3[d][1][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S3[d][1][3] / (mom_scpd * mom_scpd * mom_scpd);
                            status = status && !(theta > parsec3_l[d] && theta < parsec3_h[d]);
                        }
                    }
                    ...
                    // sector 4 has two gaps
                        ...
                        double parsec4_l[2], parsec4_h[2];
                        for (int d = 0; d < 2; d++) {
                            parsec4_l[d] = fid_1gev_750_pfid_S4[d][0][0] + fid_1gev_750_pfid_S4[d][0][1] / mom_scpd +
                                           fid_1gev_750_pfid_S4[d][0][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S4[d][0][3] / (mom_scpd * mom_scpd * mom_scpd);
                            parsec4_h[d] = fid_1gev_750_pfid_S4[d][1][0] + fid_1gev_750_pfid_S4[d][1][1] / mom_scpd +
                                           fid_1gev_750_pfid_S4[d][1][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S4[d][1][3] / (mom_scpd * mom_scpd * mom_scpd);
                            status = status && !(theta > parsec4_l[d] && theta < parsec4_h[d]);
                        }
                    }

                    // sector 5 has four gaps
                    else if (tsector == 5) { // the fourth bad TOF pd. can be seen only below b=0.3 and so there are
                                             // just three bad TOFs for p
                        double parsec5_l[3], parsec5_h[3];
                        for (int d = 0; d < 3; d++) {
                            ...
                            if (d == 2 && mom_scpd < 0.5)
                                mom_scpd = 0.5;
                            parsec5_l[d] = fid_1gev_750_pfid_S5[d][0][0] + fid_1gev_750_pfid_S5[d][0][1] / mom_scpd +
                                           fid_1gev_750_pfid_S5[d][0][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S5[d][0][3] / (mom_scpd * mom_scpd * mom_scpd);
                            parsec5_h[d] = fid_1gev_750_pfid_S5[d][1][0] + fid_1gev_750_pfid_S5[d][1][1] / mom_scpd +
                                           fid_1gev_750_pfid_S5[d][1][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S5[d][1][3] / (mom_scpd * mom_scpd * mom_scpd);
                            status = status && !(theta > parsec5_l[d] && theta < parsec5_h[d]);
                        }
                    }
                    ...
                    // sector 6 has two gaps
                        ...
                        for (int d = 0; d < 2; d++) {
                            ...
                            if (mom_scpd > 0.6)
                                mom_scpd = 0.6;
                            else if (mom_scpd < 0.3)
                                mom_scpd = 0.3;
                            parsec6_l[d] = fid_1gev_750_pfid_S6[d][0][0] + fid_1gev_750_pfid_S6[d][0][1] / mom_scpd +
                                           fid_1gev_750_pfid_S6[d][0][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S6[d][0][3] / (mom_scpd * mom_scpd * mom_scpd);
                            parsec6_h[d] = fid_1gev_750_pfid_S6[d][1][0] + fid_1gev_750_pfid_S6[d][1][1] / mom_scpd +
                                           fid_1gev_750_pfid_S6[d][1][2] / (mom_scpd * mom_scpd) +
                                           fid_1gev_750_pfid_S6[d][1][3] / (mom_scpd * mom_scpd * mom_scpd);
        ...
        if (en_beam[fbeam_en] > 2. && en_beam[fbeam_en] < 3. && fTorusCurrent > 2240 && fTorusCurrent < 2260) {
            ...
            if (mom_for < 0.3)
                mom_for = 0.3; // momentum smaller than 300 MeV/c, use 300 MeV/c
            if (mom_for > 1.6)
                mom_for = 1.6;   // momentum greater than 1.6 GeV/c, use 1.6 GeV/c
            Float_t mom_bak = p; // momentum for backward constraints
            if (mom_bak < 0.2)
                mom_bak = 0.2; // momentum smaller than 200 MeV/c, use 200 MeV/c
            if (mom_bak > 1.0)
                mom_bak = 1.0; // momentum greater than 1.0 GeV/c, use 1.0 GeV/c
            Float_t theta0 = 8.5;
        ...
        if (en_beam[fbeam_en] > 4. && en_beam[fbeam_en] < 5. && fTorusCurrent > 2240 &&
            fTorusCurrent < 2260) { // 4 GeV Fiducial Cut Rustam Niyazov
            ...
            if (p > 4.4)
                p = 4.4; // momentum greater than 4.4 GeV/c, use 4.4 GeV/c

            // get parametrized values of theta_max for p<0.6 GeV/c region
            if (p < 0.6) {
                theta_max = fgPar_4Gev_2250_Pfidbl[sector][4] + fgPar_4Gev_2250_Pfidbl[sector][5] * p;
            }
            // get parametrized values of theta_max for p>0.6 GeV/c region
            else {
                theta_max = fgPar_4Gev_2250_Pfidbl[sector][4] + fgPar_4Gev_2250_Pfidbl[sector][5] * p1;
            }
            ...
            phi45l = parfidl[0] * (parfidl[2] - 45) /
                     (45 - parfidl[2] + (parfidl[1] / parfidl[0])); // parametrized value of phi at theta=45 deg.
            phi45r = -parfidr[0] * (parfidr[2] - 45) / (45 - parfidr[2] + (parfidr[1] / parfidr[0]));
            if (theta > thetab) { // backward region defined by theta >45 deg.
                if (theta > 140)
                    theta = 140; // theta greater than 140 degrees, use 140 degrees
                if (p > 1)
                    p = 1.; // momentum greater than 1.0 GeV/c, use 1.0 GeV/c
            ...
            if (Forward) { // Forward region
                ...
                else if (theta >= 60 && theta <= dl) {
                    cphil = phi60l;
                } // phi=constant
                else if (theta > dl && theta <= theta_max) {
                    cphil = (140 - theta) * (phi60l - el) / (140 - dl) + el;
                } // phi=stright line
                else if (theta > theta_max) {
                    cphil = 0;
                } // cut out if theta>theta_max
                // BackwardB region & phi>0
                if (theta >= 60 && theta <= dr) {
                    cphir = phi60r;
                } // phi=constant
                else if (theta > dr && theta <= theta_max) {
                    cphir = (140 - theta) * (phi60r - er) / (140 - dr) + er;
                } // phi=stright line
                else if (theta > theta_max) {
                    cphir = 0;
                } // cut out if theta>theta_max
            ...
            if (phi < 0)
                status = (phi > cphil); // check the constrains
            else if (phi >= 0) {
                status = (phi < cphir);
            }

            if (theta < theta_min)
                status = kFALSE; // Cutting out events below theta_min

            if (Forward && p < 0.6 && theta < 20.6 - 11.4 * p)
                status = kFALSE; // function defines cut of the edge at low theta for p<0.6 GeV/c
            ...
            if (status && SCpdcut) { // cut bad scintillator paddles
                if (p < 1.0) {
                    ...
                    Float_t mom_scpd = p; // momentum for bad sc paddles cuts
                    if (mom_scpd < 0.3)
                        mom_scpd = 0.3;     // momentum smaller than 200 MeV/c, use 200 MeV/c
                    if (tsector == 2) {     // sector 2 has one bad paddle
                        Float_t badpar2[2]; // 2 parameters to determine the position of the theta gap
                        for (Int_t i = 0; i < 2; i++) {
                            badpar2[i] = 0;
                    ...
                    if (tsector == 2) {
                        ...
                        for (int i = 0; i < 2; i++) {
                            badpar2[i] = 0;
                            // calculate the parameters using 1/p
                            badpar2[i] = fgPar_4Gev_2250_Pfid_ScpdS2_extra[i][0] +
                                         fgPar_4Gev_2250_Pfid_ScpdS2_extra[i][1] / mom_scpd +
                                         fgPar_4Gev_2250_Pfid_ScpdS2_extra[i][2] / (mom_scpd * mom_scpd) +
                                         fgPar_4Gev_2250_Pfid_ScpdS2_extra[i][3] / (mom_scpd * mom_scpd * mom_scpd);
                        }
                    ...
                    if (tsector == 3) {
                        ...
                        for (int i = 0; i < 8; i++) {
                            badpar3[i] = 0;
                            // calculate the parameters using 1/p
                            badpar3[i] = fgPar_4Gev_2250_Pfid_ScpdS3_extra[i][0] +
                                         fgPar_4Gev_2250_Pfid_ScpdS3_extra[i][1] / mom_scpd +
                                         fgPar_4Gev_2250_Pfid_ScpdS3_extra[i][2] / (mom_scpd * mom_scpd) +
                                         fgPar_4Gev_2250_Pfid_ScpdS3_extra[i][3] / (mom_scpd * mom_scpd * mom_scpd);
                        }
                    ...
                    // sector 4 has two bad paddle
                        ...
                        for (int i = 0; i < 4; i++) {
                            badpar4[i] = 0;
                            // calculate the parameters using 1/p
                            badpar4[i] = fgPar_4Gev_2250_Pfid_ScpdS4_extra[i][0] +
                                         fgPar_4Gev_2250_Pfid_ScpdS4_extra[i][1] / mom_scpd +
                                         fgPar_4Gev_2250_Pfid_ScpdS4_extra[i][2] / (mom_scpd * mom_scpd) +
                                         fgPar_4Gev_2250_Pfid_ScpdS4_extra[i][3] / (mom_scpd * mom_scpd * mom_scpd);
                        }
                        for (int ipar = 0; ipar < 2; ipar++)
                            status = status && !(theta > badpar4[2 * ipar] && theta < badpar4[2 * ipar + 1]);
                    }
                    // sector 5 has four bad paddles
                        ...
                        for (Int_t i = 0; i < 8; i++) {
                            badpar5[i] = 0;
                            // calculate the parameters using 1/p
                            badpar5[i] = fgPar_4Gev_2250_Pfid_ScpdS5_extra[i][0] +
                                         fgPar_4Gev_2250_Pfid_ScpdS5_extra[i][1] / mom_scpd +
                                         fgPar_4Gev_2250_Pfid_ScpdS5_extra[i][2] / (mom_scpd * mom_scpd) +
                                         fgPar_4Gev_2250_Pfid_ScpdS5_extra[i][3] / (mom_scpd * mom_scpd * mom_scpd);
                        }
                        for (Int_t ipar = 0; ipar < 4; ipar++)
                            status = status && !(theta > badpar5[2 * ipar] && theta < badpar5[2 * ipar + 1]);
                    }
    ...
    if (beam_en == "") {
        ...
        if (en_beam[fbeam_en] > 1. && en_beam[fbeam_en] < 2.) {
            ...
            if (fTorusCurrent < 1510 && fTorusCurrent > 1490) {
                ...
                if (!PiplFiducialCutExtra(fbeam_en, momentum)) {
                    status = false;
                }
            ...
            if (fTorusCurrent < 760 && fTorusCurrent > 740) {
                ...
                if (status && SCpdcut) { // cut bad scintillator paddles
                    Int_t tsector = sector + 1;
                    Float_t mom_scpd = momentum.Mag(); // momentum for bad sc paddles cuts
                    // F.H. 10/31/19 Mariana's update mom_scpd =momentum.Mag() compared to mom_scpd= p before; but this
                    // skips if conditions above on "p" NOT USED by Mariana's update  if (mom_scpd>1.0)mom_scpd=1.0; //
                    // momentum greater than 1000 MeV/c, use 1000 MeV/c
                    if (mom_scpd < 0.2)
                        mom_scpd = 0.2; // momentum smaller than 200 MeV/c, use 200 MeV/c
...
//                   Uses 2 GeV pimi fiducial cuts for both 2 and 4 GeV analysis, from electron cuts plus low momentum
//                   extension October 27th, 2020 - Theta gap parameters at 1 GeV (750A), and 2/4 GeV (2250A) updated to
//                   replace reuse of
//                                        electron gap functions, which are too messy to reuse accurately and account
//                                        for CC cuts inappropriate to pions This update also includes Florian's update
//                                        to loosen the constraint on upper limit of theta, which at high momentum is
//                                        still much too tight A future update will implement new phi fiducial cuts at
//                                        all momenta, but the current bug fixes are considered adequate for the zero
//                                        pion analyses
...
Bool_t Fiducial::PimiFiducialCut(std::string beam_en, TVector3 momentum, Float_t *pimi_philow, Float_t *pimi_phiup) {
    ...
    if (beam_en == "") {
        ...
        phi_deg -= sector * 60; // fiducial cuts defined using individual sector distributions, redrawn to be centred on
                                // 0 degrees in phi. This angle allows those cut definitions to be applied
        ...
        if (en_beam[fbeam_en] > 1. && en_beam[fbeam_en] < 2.) {
            ...
            if (fTorusCurrent > 1490 && fTorusCurrent < 1510) {
                ...
                for (Int_t mompar = 0; mompar < 6; mompar++) {
                    for (Int_t phipar = 0; phipar < 5; phipar++) {
                        ...
                        // std::cout << mom_e << " " << mompar << " " << phipar << " " << phipars[1] << " " <<
                        // phipars[2] << " " << phipars[3] << " " << phipars[4] << " " << phipars[5] << std::endl;
                    }
                }
                ...
                {
                    bool SCpdcut = true;
                    if (SCpdcut) { // if the SCpdCut bit is set, take off the bad SC paddle by strictly cutting off a
                                   // theta gap.
                        if (status) {
                            // sector 3 has two bad paddles
                            if (tsector == 3) {
                            ...
                            // sector 4 has one bad paddle
                                ...
                                for (int i = 0; i < 2; i++) {
                                    ...
                                    // calculate the parameters using pol7
                                    for (int d = 7; d >= 0; d--) {
                                        badpar4[i] = badpar4[i] * mom_pi + fgPar_1gev_1500_Pimfid_Theta_S4[i][d];
                                    }
                                }
                            ...
                            // sector 5 has four bad paddles
                                ...
                                for (Int_t i = 0; i < 8; i++) {
                                    ...
                                    // calculate the parameters using pol7
                                    for (Int_t d = 7; d >= 0; d--) {
                                        badpar5[i] = badpar5[i] * mom_pi + fgPar_1gev_1500_Pimfid_Theta_S5[i][d];
                                    }
                                }
                                if (mom_pi < 1.25)
                                    badpar5[0] = 23.4 * 1500 / 2250;
                    ...
                    if (!PimiFiducialCutExtra(fbeam_en, momentum)) {
                        status = false;
                    }
            ...
            if (fTorusCurrent > 740 && fTorusCurrent < 760) {
                ...
                // Temporary momentum parameter to define a maximum limit on theta in the range 0.1-0.7 GeV pion
                // momentum Very few events 700 MeV at 1.1 GeV beam energy, and the momentum threshold for pion
                // detection is 150 MeV All this does is remove some insignificant scruff
                if (p_theta > 0.7)
                    p_theta = 0.7;
                else if (p_theta < 0.1)
                    p_theta = 0.1;
                ...
                ////////////////////////////////////////////////////////////////////////////////////////////
                bool SCpdcut = true;
                if (SCpdcut) { // if the SCpdCut bit is set, take off the bad SC paddle by strictly cutting off a theta
                               // gap.
                    if (status) {
                        mom_pi = mom.Mag();
                        // Reset ignores mom_e > 1.1 cut from above. Is this okay? F.H. 10/31/19  - Yes, as it applies
                        // the theta cut functions at all momenta SF 10/11/20
                        ...
                        // Using a set of fid_1gev_750_pimifid_S[1,6] parameters, hard coded in Fiducial.h, to avoid
                        // confusion with electron, althoug some parameters are the same sector 1 has one gap
                        if (tsector == 1) {
                            double parsec1_l, parsec1_h;
                            // Evaluate theta gap boundaries from saved parameters
                            parsec1_l = fid_1gev_750_pimifid_S1[0][0] + fid_1gev_750_pimifid_S1[0][1] / mom_pi +
                                        fid_1gev_750_pimifid_S1[0][2] / (mom_pi * mom_pi) +
                                        fid_1gev_750_pimifid_S1[0][3] / (mom_pi * mom_pi * mom_pi);
                            parsec1_h = fid_1gev_750_pimifid_S1[1][0] + fid_1gev_750_pimifid_S1[1][1] / mom_pi +
                                        fid_1gev_750_pimifid_S1[1][2] / (mom_pi * mom_pi) +
                                        fid_1gev_750_pimifid_S1[1][3] / (mom_pi * mom_pi * mom_pi);

                            status = status && !(theta_deg > parsec1_l && theta_deg < parsec1_h);
                        }
                        // //sector 2 has one gap, however, it doesn't seem to be present in pi minus, it may be a CC
                        // gap and only applicable to electrons
                        //           if(tsector == 2){
                        //             double parsec2_l,parsec2_h;
                        //             if(mom_pi<0.4) mom_pi=0.4;
                        // 	    //Evaluate theta gap boundaries from saved parameters
                        //             parsec2_l= fid_1gev_750_pimifid_S2[0][0]+fid_1gev_750_pimifid_S2[0][1]/mom_pi
                        //             +fid_1gev_750_pimifid_S2[0][2]/(mom_pi*mom_pi)
                        //             +fid_1gev_750_pimifid_S2[0][3]/(mom_pi*mom_pi*mom_pi); parsec2_h=
                        //             fid_1gev_750_pimifid_S2[1][0]+fid_1gev_750_pimifid_S2[1][1]/mom_pi
                        //             +fid_1gev_750_pimifid_S2[1][2]/(mom_pi*mom_pi)
                        //             +fid_1gev_750_pimifid_S2[1][3]/(mom_pi*mom_pi*mom_pi);
                        ...
                        // sector 4 has two gaps, however, the first gap is probably a CC gap, and not applicable to the
                        // pi minus, an additional TOF gap replaces this
                        else if (tsector == 4) {
                            double parsec4_l[2], parsec4_h[2];
                            for (int d = 0; d < 2; d++) {
                                // Evaluate theta gap boundaries from saved parameters
        ...
        //--------------------------------------------------------------
        // 2.2 and 4.4 GeV - uses 2.2 GeV electron cuts, plus a weird fudge at pion momenta below 350 MeV //This MUST be
        // replaced by bespoke cuts
        //--------------------------------------------------------------
        if (en_beam[fbeam_en] > 2. && en_beam[fbeam_en] < 5. && fTorusCurrent > 2240 && fTorusCurrent < 2260) {

            Float_t phimin, phimax;

            //        Float_t par[6];               // six parameters to determine the outline of Theta vs Phi  //why
            //        six parameters? it's five everywhere else...
            // p_theta == mom_pi
            // theta maximum limit functions only defined in the momentum range 0.1 - 2.075 //is this an appropriate
            // maximum at 4 GeV?
            if (p_theta > 2.075) {
                p_theta = 2.075;
            } else if (p_theta < 0.1) {
            ...
            if (mom_pi > 0.35) { // theta vs phi outline for high p region obtained by Bin
                ...
                if (mom_pi > 2.) {
                    mom_pi = 2.; // to extrapolate the cut to higher momenta for pimi //(to badly extrapolate) S.F.
                                 // October 2020
                }

                Float_t par[6]; // six parameters to determine the outline of Theta vs Phi  //why six parameters? it's
            ...
            //---The pi- parameters obtained only at low momentum
                ...
                if (mom_pi > 0.325)
                    mom_pi = 0.325; // This causes the notch at the lower theta limit between 325 and 350 MeV
                else if (mom_pi < 0.125)
                    mom_pi = 0.125; // This shouldn't do anything at all, as pions are selected from 150 MeV

                Float_t params[6]; // six parameters to determine the outline of Theta vs Phi, like the higher momentum
                                   // region above.  Can change to par[6] and share declaration with above
                for (Int_t i = 0; i < 6; i++) {
                    params[i] = 0;
                    for (Int_t d = 4; d >= 0; d--) {
                ...
                if (phi_deg < 0) {
                    phimin = params[3] / ((theta_deg - params[0]) + params[3] / params[1]) -
                             params[1]; // why is phimin calculated differently in the low momentum?
                    // where's phimax?
                ...
                std::cout << "pi- fiducial outline not determined" << std::endl;
            }
            ...
            // by now, we have checked if the electron is within the outline of theta vs phi plot
            if (SCpdcut) {    // if the kESCpdCut bit is set, take off the bad SC paddle by strictly cutting off a theta
                              // gap.
                if (status) { // if event not already rejected

                    //---Further gaps obtained with pi- ------
                    // Gaps obtained with pi-. Includes redefined versions of some electron gaps from
                    // fgPar_2GeV_2250_EfidThets_S{1-6}. S.F Oct 26, 2020
                    mom_pi = momentum.Mag(); // reset momentum to event value, just in case the electron paddle nonsense
                                             // causes problems
                    ...
                    // sector 4 has two gaps, one defined on pi minus, one reimplemented from electron
                        ...
                        for (int d = 0; d < 2; d++) {
                            if (d == 0) {
                                if (mom_pi < 0.15)
                                    mom_pi = 0.15; // not really needed, below pion detection threshold
                                else if (mom_pi > 0.5)
                    ...
                    // sector 6 has one gap, a gap that could be replaced by a box cut between 58 and 65 degrees to some
                    // momentum limit (probably a DC region failure)
                    if (tsector == 6) {
                        double parsec6_l, parsec6_h;
                        if (mom_pi < 0.175)
                            mom_pi = 0.175;
                        else if (mom_pi > 0.275)
                            mom_pi = 0.275;
                        parsec6_l = fid_2gev_2250_pimifid_S6[0][0] + fid_2gev_2250_pimifid_S6[0][1] / mom_pi +
                                    fid_2gev_2250_pimifid_S6[0][2] / (mom_pi * mom_pi) +
                                    fid_2gev_2250_pimifid_S6[0][3] / (mom_pi * mom_pi * mom_pi);
                        parsec6_h = fid_2gev_2250_pimifid_S6[1][0] + fid_2gev_2250_pimifid_S6[1][1] / mom_pi +
                                    fid_2gev_2250_pimifid_S6[1][2] / (mom_pi * mom_pi) +
                                    fid_2gev_2250_pimifid_S6[1][3] / (mom_pi * mom_pi * mom_pi);
...
bool Fiducial::Phot_fid(TVector3 V3_phot) {
    ...
    if ((costheta > low_lim1_ec->Eval(phi_deg) && costheta < up_lim1_ec->Eval(phi_deg) &&
        ...
        (costheta > low_lim2_ec->Eval(phi_deg) && costheta < up_lim2_ec->Eval(phi_deg) &&
         costheta < rightside_lim2_ec->Eval(phi_deg) && costheta < leftside_lim2_ec->Eval(phi_deg)) ||
        (costheta > low_lim3_ec->Eval(phi_deg) && costheta < up_lim3_ec->Eval(phi_deg) &&
         costheta < rightside_lim3_ec->Eval(phi_deg) && costheta < leftside_lim3_ec->Eval(phi_deg)) ||
        (costheta > low_lim4_ec->Eval(phi_deg) && costheta < up_lim4_ec->Eval(phi_deg) &&
         costheta < rightside_lim4_ec->Eval(phi_deg) && costheta < leftside_lim4_ec->Eval(phi_deg) && !hot_spot) ||
        (costheta > low_lim5_ec->Eval(phi_deg) && costheta < up_lim5_ec->Eval(phi_deg) &&
         costheta < rightside_lim5_ec->Eval(phi_deg) && costheta < leftside_lim5_ec->Eval(phi_deg)) ||
        (costheta > low_lim6_ec->Eval(phi_deg) && costheta < up_lim6_ec->Eval(phi_deg) &&
         costheta < rightside_lim6_ec->Eval(phi_deg) && costheta < leftside_lim6_ec->Eval(phi_deg))) { // EC only
        status = true;
    } else {
        status = false;
    }
...
bool Fiducial::Pi_phot_fid_united(std::string beam_en, TVector3 V3_pi_phot, int q_pi_phot) {
    ...
    if (q_pi_phot == 0)
        status = Phot_fid(V3_pi_phot);
    if (q_pi_phot > 0)
        status = PiplFiducialCut(fbeam_en, V3_pi_phot, &pi_cphil, &pi_cphir);
    if (q_pi_phot < 0)
        status = PimiFiducialCut(fbeam_en, V3_pi_phot, &pi_phimin, &pi_phimax);
    return status;
}
...
bool Fiducial::Pi_phot_fid_unitedExtra(std::string beam_en, TVector3 V3_pi_phot, int q_pi_phot) {
    ...
    if (q_pi_phot == 0)
        status = Phot_fidExtra(V3_pi_phot);
    if (q_pi_phot > 0)
        status = PiplFiducialCutExtra(fbeam_en, V3_pi_phot);
    if (q_pi_phot < 0)
        status = PimiFiducialCutExtra(fbeam_en, V3_pi_phot);
    return status;
}
